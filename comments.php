...
// Do not delete these lines
	...
	if ( post_password_required() ) { ?>
		<p class="nocomments"><?php _e('This post is password protected. Enter the password to view comments.', 'paramitopia'); ?></p>
	<?php
		return;
	}
?>
<!-- You can start editing here. -->
	...
	<?php if ( have_comments() ) { ?>
			<div id="comments-div"><?php if ( comments_open() ) { ?><span id="comments-addcomment"><a href="#respond"  rel="nofollow" title="<?php _e('Leave a comment ?', 'paramitopia'); ?>"><?php _e('Leave a comment ?', 'paramitopia'); ?></a></span><?php } ?><h2 id="comments"><?php comments_number('0', '1', '%' );?> <?php _e('Comments.','paramitopia'); ?></h2></div>
			<ol class="commentlist" id="thecomments">
					<?php wp_list_comments('type=comment&callback=paramitopia_mytheme_comment'); ?>
			</ol>
			<div class="navigation"><?php paginate_comments_links(); ?></div>
	<?php } else { // this is displayed if there are no comments so far ?>
	<?php if ( ! comments_open() && !is_page() ) { ?>
			<div id="comments-div"><h2 id="comments"><?php _e('Comments are closed.','paramitopia'); ?></h2></div>
...
if ($options['smilies'] != 'true') { ?>
	...
	$comment_notes='<p class="comment-note">' . __('NOTE - You can use these ','paramitopia') . sprintf(('<abbr title="HyperText Markup Language">HTML</abbr> tags and attributes:<br />%s' ), ' <code>' . allowed_tags() . '</code>' ) . '</p>';
	if($options['comment_notes']=='true') $comment_notes='';
	$fields =  array(
			'author' => '<p class="comment-form-author">' .
			'<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" />'. ' <label for="author"><small>' . __( 'NAME','paramitopia' ) . '</small></label></p>',
			'email'  => '<p class="comment-form-email">' .
			'<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" /> <label for="email">' . __( 'EMAIL', 'paramitopia' ) . '</label></p>',
			'url'    => '<p class="comment-form-url">' .
			'<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /> <label for="url">' . __( 'Website URL', 'paramitopia' ) . '</label></p>',
			);
	$args = array(
			...
			'title_reply'          => __( 'Leave a Comment', 'paramitopia'),
			'title_reply_to'       => __('Reply to %s &not;<br />','paramitopia'), 
			'cancel_reply_link'    => __( '<small>Cancel reply</small>', 'paramitopia' ),
			'label_submit'         => __( 'SUBMIT', 'paramitopia' )
			);
	...
	if($havepings == 1) : ?>
		<div class="trackbacks-pingbacks">
			<h3><?php _e('Trackbacks and Pingbacks:', 'paramitopia'); ?></h3>
			<ul id="pinglist">
				<?php wp_list_comments('type=pings&per_page=0&callback=paramitopia_custom_pings'); ?>
			</ul>
		</div>
	<?php endif; ?>
